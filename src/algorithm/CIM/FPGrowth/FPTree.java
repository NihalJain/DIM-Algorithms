package algorithm.CIM.FPGrowth;
...
public class FPTree {
	...
	// root of the tree
	FPNode root = new FPNode();//countOfUniqueItems); // null node
	...
	public void addTransaction(List<Integer> transaction) {
		...
		for(Integer item : transaction){
			...
			if(child == null){ 
				...
				FPNode newNode = new FPNode();//currentNode.itemID + 1);
				newNode.itemID = item;
                ...
                if (currentNode != root) {
                    newNode.bitMap = (BitSet) currentNode.bitMap.clone();
                ...
                /*if (currentNode != root) {
                    ...
                    //child.bitMap = (BitSet)(currentNode.bitMap.clone());
                }*/
                ...
                //System.out.println("BitMap:"+currentNode.bitMap);
	}
	...
	void addPrefixPath(List<FPNode> prefixPath, Map<Integer, Integer> mapSupportBeta, int relativeMinsupp) {
		...
		for(int i = prefixPath.size() -1; i >=1; i--){ 
			...
			if(mapSupportBeta.get(pathItem.itemID) >= relativeMinsupp){
				...
				if(child == null){ 
					// there is no node, we create a new one
					FPNode newNode = new FPNode();//countOfUniqueItems);
